...
def run_action_list_on_emulator_and_save_video(action_list):

    max_steps = len(action_list) - 1
    env_config = {
            'headless': True, 'save_final_state': True, 'early_stop': False,
            'action_freq': 24, 'init_state': './has_pokedex_nballs.state', 'max_steps': max_steps, #ep_length, 
            'print_rewards': False, 'save_video': True, 'fast_video': False, 'session_path': Path("golden_session"),
            'gb_path': './PokemonRed.gb', 'debug': False, 'sim_frame_dist': 2_000_000.0, 'instance_id': 'golden_recorded'
    }
